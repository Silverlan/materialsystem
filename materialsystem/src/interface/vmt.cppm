...
export namespace pragma::material {
	...
	bool vmt_parameter_to_numeric_type(const std::string &value, T &outResult)
	{
		if constexpr(std::is_same_v<T, bool>) {
			outResult = pragma::util::to_boolean(value);
			return true;
		}
		else if constexpr(std::is_integral_v<T>) {
			outResult = pragma::util::to_int(value);
			return true;
		}
		else if constexpr(std::is_floating_point_v<T>) {
			outResult = pragma::util::to_float(value);
			return true;
		}
		return false;
	}
	...
	std::optional<Vector3> vmt_parameter_to_color(std::string value)
	{
		...
		pragma::string::remove_whitespace(value);
	...
	void get_vmt_data(const std::shared_ptr<datasystem::Block> &root, datasystem::Settings &dataSettings, const std::string &key, VTFLib::Nodes::CVMTNode *node, const std::function<TInternal(TInternal)> &translate = nullptr)
	{
		auto type = node->GetType();
		switch(type) {
		case NODE_TYPE_SINGLE:
	...
	template<class TData, typename TInternal>
	void get_vmt_data(const std::shared_ptr<datasystem::Block> &root, datasystem::Settings &dataSettings, const std::string &key, std::string value, const std::function<TInternal(TInternal)> &translate = nullptr)
	{
		if(translate != nullptr) {
