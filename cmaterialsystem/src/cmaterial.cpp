...
CMaterial::CallbackInfo::CallbackInfo(const std::function<void(std::shared_ptr<Texture>)> &_onload) : count(0)
{
	if(_onload) {
...
bool CMaterial::ShaderEqualFn::operator()(const util::WeakHandle<prosper::Shader> &whShader0, const util::WeakHandle<prosper::Shader> &whShader1) const { return whShader0.get() == whShader1.get(); }
...
CMaterial::CMaterial(msys::MaterialManager &manager) : Material(manager) {}
CMaterial::CMaterial(msys::MaterialManager &manager, const util::WeakHandle<util::ShaderInfo> &shader, const std::shared_ptr<ds::Block> &data) : Material(manager, shader, data)
{
	// UpdatePrimaryShader();
}
CMaterial::CMaterial(msys::MaterialManager &manager, const std::string &shader, const std::shared_ptr<ds::Block> &data) : Material(manager, shader, data)
{
	// UpdatePrimaryShader();
}
...
std::unordered_map<util::WeakHandle<prosper::Shader>, std::shared_ptr<prosper::IDescriptorSetGroup>, CMaterial::ShaderHash, CMaterial::ShaderEqualFn>::const_iterator CMaterial::FindShaderDescriptorSetGroup(prosper::Shader &shader) const
{
...
void CMaterial::LoadTexture(const std::shared_ptr<ds::Block> &data, const std::shared_ptr<ds::Texture> &dataTexture, TextureLoadFlags loadFlags, const std::shared_ptr<CallbackInfo> &callbackInfo) { LoadTexture(data, dataTexture->GetValue(), loadFlags, callbackInfo); }
